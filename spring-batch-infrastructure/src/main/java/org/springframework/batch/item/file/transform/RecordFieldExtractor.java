...
/**
 * This is a field extractor for a java record. By default, it will extract all 
 * record components, unless a subset is selected using {@code setNames(String[])}.
 * 
...
public class RecordFieldExtractor<T> implements FieldExtractor<T> {
	...
	/**
	 * Set record component names to extract.
	 * 
	 * @param names record component names to be extracted by the {@link #extract(Object)} method.
	 */
	public void setNames(String[] names) {
		Assert.notNull(names, "Names must be non-null");
		Assert.notEmpty(names, "Names must be empty");
		validate(names);
	...
	public Object[] extract(T item) {
		...
		for (String componentName : this.names) {
			...
			try {
				...
				throw new RuntimeException("Unable to extract value for component " + componentName, e);
			}
		}
		return values.toArray();
	}

	private String[] getRecordComponentNames() {
		String[] recordComponentNames = new String[recordComponents.length];
		for (int i = 0; i < recordComponents.length; i++) {
			recordComponentNames[i] = recordComponents[i].getName();
		}
		...
		for (String name : names) {
			if (getRecordComponentByName(name) == null) {
				throw new IllegalArgumentException("Name '" + name +
						"' is not defined in record " + targetType.getName());
			}
		}
	}
	...
	private RecordComponent getRecordComponentByName(String name) {
    	// TODO can be optimized using an instance variable map
